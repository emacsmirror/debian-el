    ...
    (cond
     (new-archive-format
      ...
      (cond
       ...
       ((and (goto-char 1)(re-search-forward "data.tar.zst" nil t))
        (erase-buffer)
        (call-process "ar" nil '(t t) nil "-p" debfile "data.tar.zst")
        (goto-char (point-max))
        (when (search-backward "is not a valid archive" nil t)
          (kill-buffer data-buffer)
          (kill-buffer info-buffer)
          (error "%s: Not a valid package file" deb-view-buffer-name))
        (call-process-region (point-min) (point-max) "zstd" t t nil "-cd"))
